...
| 能力 | 状态 | 说明 |
| --- | --- | --- |
| Codex OAuth 设备码登录 | 已发布 `v2.6.0.znmlr.1` | 认证、加密凭据、刷新、安全 IPC、Codex Responses、固定模型目录、设备码设置页、编译期开关和下游版本规则已经整合；真机登录、问答、只读工具、重启恢复及 7 个模型拉取已验证 |
...
本地 `main` 已合入上游 [`v2.6.2`](https://github.com/Mangi-11/Eta/releases/tag/v2.6.2)（`bd4a14ceeda81b9063b9fde91b14e47f3851929f`），并已发布 [`v2.6.2.znmlr.1`](https://github.com/yangyunzhao/Eta/releases/tag/v2.6.2.znmlr.1)（`versionCode 26201`）。2026-08-24 已按最新官方 Codex CLI `rust-v0.149.1`（peeled `ff29a44391deccde0aba0f8390337d7f3c319ea4`）重新核对设备码、刷新、Responses 与模型目录契约：无必须迁移的变化，因此 `CODEX_PROTOCOL_COMPAT_VERSION` 保持代表已完整验证的 `0.147.0` 基线。main 与 tag CI 均已通过，最终 CI Release APK 的 SHA-256 为 `2BB9C10E8A3A484511433814FCB7B5C5BFE344E93D951022455E292EE6B314E1`。
...
- 当前已发布 `v2.6.0.znmlr.1`，对应 `versionName 2.6.0.znmlr.1` 和 `versionCode 26001`。
