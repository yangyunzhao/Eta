...
| Task 9 / Step 7 | 已完成（含缺口） | 已记录候选提交、真机成功项、CI 结果及脱敏失败类别；AndroidKeyStore instrumentation 与注销敏感日志计数未执行，作为发布已知验证缺口，不得追记为通过。 |
...
| Task 14 / Step 4 | 已完成 | main/tag CI 与最终 CI Release APK 核验成功后，`v2.6.5.znmlr.1` 已推送并正式发布；本地/CI 产物均确认 applicationId `fuck.andes`、`26501` 与 znmlr 发布证书。 |
...
### Task 14: 上游 v2.6.5 同步与候选验证

- [x] **Step 1: 精确合入上游 release tag**

从 `upstream` 获取并普通合入轻量 tag `v2.6.5` 的 `bf0c6fee5968b1f1f31ec4dece1201082d17226c`，不合入其后的 `upstream/main` 提交。

- [x] **Step 2: 保留 MCP/恢复能力与 OAuth 数据路径**

Room 升至 v19，支持下游 v16 与上游 v18 的历史数据库；共享 Responses 解析器纳入上游多 block、终态和 MCP hosted 工具生命周期，同时保留 Codex 固定 HTTPS、OAuth 凭据和调试脱敏边界。

- [x] **Step 3: 完成本地候选自动验证**

Kotlin 编译、MCP/恢复链/OAuth 定向回归、Lint 与 Debug APK 构建通过；完整 JVM 764 项仍仅有 8 个既有 Windows/Robolectric/POSIX 基线失败。随后 main/tag CI 通过并已正式发布 `2.6.5.znmlr.1`。

- [x] **Step 4: 构建并核验本地签名 Release 候选**

构建 `:app:assembleRelease`，确认 applicationId `fuck.andes`、版本 `2.6.5.znmlr.1` / `26501`、APK Signature Scheme v2 与 znmlr 发布证书；随后 main/tag CI 通过、最终 CI APK 已核验并发布 GitHub Release。
