...
class BuildVersionPolicyTest {
    ...
    fun `当前构建使用首个 znmlr 下游候选版本`() {
        assertEquals("2.6.0.znmlr.1", BuildConfig.VERSION_NAME)
        assertEquals(26_001, BuildConfig.VERSION_CODE)
    }
    ...
    fun `下游版本由已验证的上游基线和发布序号计算`() {
        ...
        assertEquals("2.6.0", upstreamVersionName)
        assertEquals(260, upstreamVersionCode)
